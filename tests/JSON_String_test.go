...
func TestCanParseString(t *testing.T) {
	...
	if !json_obj.HasKey("key") {
		t.Errorf("key not found available keys are %s", json_obj.Keys())
...
func TestCanParseStringWithLeadingTrailingLineBreakLiteralAndReal(t *testing.T) {
	json_obj := ParseJSONSuccessfully(t, "{\\\n\"key\":\"value\"\\\n}")
...
func TestCanParseNestedJSONString(t *testing.T) {
	...
	if !json_obj.HasKey("key") {
		...
		if value_errors != nil {
			...
			t.Errorf("expected: {\"inner_key\":\"inner_value\"} actual: %s", *value)
		}
	}
}
...
func TestCanParseStringWithQuoteKey(t *testing.T) {
	...
	if !json_obj.HasKey("ke\"y") {
		t.Errorf("ke\"y not found available keys are %s", json_obj.Keys())
...
func TestCanParseStringWithOpenBracketValue(t *testing.T) {
	...
	if !json_obj.HasKey("key") {
		...
		if value_errors != nil {
			...
			t.Errorf("expected: val{ue actual: %s", *value)
		}
	}
}
